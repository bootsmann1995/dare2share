		...
		<div class="max-w-md w-full space-y-8">
			...
			>
				<div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
					<div class="mt-3">
						<h3 class="text-lg font-medium text-gray-900 text-center mb-4">Reset your password</h3>
...
onMounted(() => {
	...
	if (urlError) {
		if (errorCode === "otp_expired" || urlError === "access_denied") {
			error.value =
				"The email link is invalid or has expired. Please request a new password reset or verification email.";
...
const signInWithGoogle = async () => {
	try {
		const { getAuthRedirectUrl } = useAppConfigSiteUrl();
